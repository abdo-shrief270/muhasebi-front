<template>
  <div>
    <NuxtLayout name="auth">
      <div class="mb-8">
        <h2 class="text-2xl font-bold text-gray-800">{{ $t('auth.register') }}</h2>
        <p class="text-gray-400 mt-1 text-sm">
          {{ locale === 'ar' ? 'أنشئ حسابك وابدأ في إدارة أعمالك' : 'Create your account and start managing your business' }}
        </p>
      </div>
      ...
      <form @submit.prevent="handleRegister" class="space-y-5">
        <!-- Company Name -->
        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1.5">{{ $t('auth.companyName') }}</label>
          <div class="relative">
            ...
            <input
              ...
              :placeholder="locale === 'ar' ? 'اسم الشركة أو المكتب' : 'Company or office name'"
            />
          </div>
          <p v-if="errors.company_name" class="text-red-500 text-xs mt-1.5">{{ errors.company_name[0] }}</p>
        </div>

        <!-- Full Name -->
        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1.5">{{ $t('auth.fullName') }}</label>
          <div class="relative">
            <span class="absolute start-4 top-1/2 -translate-y-1/2 text-gray-300">
        ...
        <!-- Password -->
        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1.5">{{ $t('auth.password') }}</label>
          <div class="relative">
            <span class="absolute start-4 top-1/2 -translate-y-1/2 text-gray-300">
        ...
        <!-- Confirm Password -->
        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1.5">{{ $t('auth.confirmPassword') }}</label>
          <div class="relative">
            <span class="absolute start-4 top-1/2 -translate-y-1/2 text-gray-300">
      ...
      <p class="mt-6 text-center text-sm text-gray-500">
        {{ $t('auth.hasAccount') }}
        <NuxtLink to="/auth/login" class="text-secondary-400 hover:text-secondary-500 font-medium">
          {{ $t('auth.login') }}
        </NuxtLink>
      </p>
    </NuxtLayout>
...
async function handleRegister() {
  ...
  try {
    ...
    errorMessage.value = error.data?.message || (locale.value === 'ar' ? 'فشل إنشاء الحساب' : 'Registration failed')
  } finally {
    loading.value = false
  }
}
