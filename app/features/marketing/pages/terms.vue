  ...
  <div>
      <div class="max-w-3xl mx-auto px-6 py-12">
        ...
        <article v-else>
          ...
          <!-- Fallback if no backend content -->
...
const pageTitle = computed(() => {
  ...
  return locale.value === 'ar' ? 'الشروط والأحكام' : 'Terms of Service'
})
...
useHead({
  ...
  meta: [
    { name: 'description', content: page.value?.meta_description?.[locale.value === 'ar' ? 'ar' : 'en'] || '' },
  ],
})

onMounted(async () => {
  try {
    page.value = await getPage('terms')
  } catch {}
