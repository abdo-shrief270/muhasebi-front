<template>
  <FeatureBoundary id="payroll-attendance">
    <div class="px-4 lg:px-6 py-5 max-w-[1400px] mx-auto">
      <UiPageHeader
        icon="i-lucide-clock-3"
        :title="locale === 'ar' ? 'الحضور والانصراف' : 'Attendance'"
        :subtitle="locale === 'ar' ? 'سجل تسجيلات الحضور اليومية للموظفين' : 'Daily check-in / check-out records'"
      />

      <Can :perm="PERMISSIONS.MANAGE_PAYROLL">
        <template #fallback>
          <div class="bg-neutral-0 dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-12 text-center">
            <div class="w-12 h-12 mx-auto mb-4 rounded-full bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center">
              <UIcon name="i-lucide-lock" class="w-5 h-5 text-neutral-400" />
            </div>
            <p class="text-sm font-semibold text-neutral-900 dark:text-neutral-0">
              {{ locale === 'ar' ? 'لا تملك صلاحية الوصول' : "You don't have access" }}
            </p>
          </div>
        </template>

        <UiDataTable
          :columns="columns"
          :rows="rows"
          :loading="loading"
          :current-page="page"
          :total-pages="totalPages"
          empty-icon="i-lucide-clock-3"
          :empty-title="locale === 'ar' ? 'لا توجد سجلات حضور' : 'No attendance records'"
          :empty-description="locale === 'ar'
            ? 'تسجيلات الدخول والخروج اليومية ومجموع الساعات تظهر هنا.'
            : 'Daily check-ins, check-outs, and computed hours appear here.'"
          @page-change="onPage"
        >
          <template #cell-date="{ value }">
            <span class="text-sm text-neutral-700 dark:text-neutral-200 tabular-nums" dir="ltr">{{ formatDate(value) }}</span>
          </template>
          <template #cell-employee_name="{ value }">
            <span class="text-sm font-medium text-neutral-900 dark:text-neutral-0">{{ value }}</span>
          </template>
          <template #cell-check_in="{ value }">
            <span class="font-mono text-sm tabular-nums text-success-700 dark:text-success-400" dir="ltr">{{ value || '—' }}</span>
          </template>
          <template #cell-check_out="{ value }">
            <span class="font-mono text-sm tabular-nums text-warning-700 dark:text-warning-500" dir="ltr">{{ value || '—' }}</span>
          </template>
          <template #cell-hours="{ value }">
            <span class="font-mono text-sm font-medium tabular-nums text-neutral-900 dark:text-neutral-0" dir="ltr">{{ value ?? '—' }}</span>
          </template>
        </UiDataTable>
      </Can>
    </div>
  </FeatureBoundary>
</template>

<script setup lang="ts">
import Can from '~/core/rbac/Can.vue'
import { PERMISSIONS } from '~/core/rbac/permissions'

definePageMeta({ layout: 'dashboard' })
const { locale } = useI18n()
const api = useApi()
const rows = ref<any[]>([])
const loading = ref(true)
const page = ref(1)
const totalPages = ref(1)

const columns = computed(() => [
  { key: 'date', label: locale.value === 'ar' ? 'التاريخ' : 'Date' },
  { key: 'employee_name', label: locale.value === 'ar' ? 'الموظف' : 'Employee' },
  { key: 'check_in', label: locale.value === 'ar' ? 'الدخول' : 'In' },
  { key: 'check_out', label: locale.value === 'ar' ? 'الخروج' : 'Out' },
  { key: 'hours', label: locale.value === 'ar' ? 'الساعات' : 'Hours', class: 'text-end' },
])

function formatDate(d: string | null | undefined) {
  if (!d) return '—'
  try {
    return new Date(d).toLocaleDateString(locale.value === 'ar' ? 'ar-EG' : 'en-US', {
      year: 'numeric', month: 'short', day: 'numeric',
    })
  } catch { return d }
}

async function load() {
  loading.value = true
  try {
    const r: any = await api.get(`/attendance?page=${page.value}&per_page=25`).catch(() => ({ data: [], meta: { last_page: 1 } }))
    rows.value = (r.data ?? []).map((a: any) => ({ ...a, employee_name: a.employee?.name ?? '—' }))
    totalPages.value = r.meta?.last_page ?? 1
  } catch { rows.value = [] } finally { loading.value = false }
}
function onPage(p: number) { page.value = p; load() }
onMounted(load)
</script>
