<template>
  <FeatureBoundary id="payroll-loans">
    <div class="px-4 lg:px-6 py-5 max-w-[1400px] mx-auto">
      <UiPageHeader
        icon="i-lucide-hand-coins"
        :title="locale === 'ar' ? 'السلف والقروض' : 'Loans & Advances'"
        :subtitle="locale === 'ar' ? 'سلف الموظفين والأقساط الشهرية' : 'Employee advances and monthly installments'"
      />

      <Can :perm="PERMISSIONS.MANAGE_PAYROLL">
        <template #fallback>
          <div class="bg-neutral-0 dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-12 text-center">
            <div class="w-12 h-12 mx-auto mb-4 rounded-full bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center">
              <UIcon name="i-lucide-lock" class="w-5 h-5 text-neutral-400" />
            </div>
            <p class="text-sm font-semibold text-neutral-900 dark:text-neutral-0">
              {{ locale === 'ar' ? 'لا تملك صلاحية الوصول' : "You don't have access" }}
            </p>
          </div>
        </template>

        <UiDataTable
          :columns="columns"
          :rows="rows"
          :loading="loading"
          empty-icon="i-lucide-hand-coins"
          :empty-title="locale === 'ar' ? 'لا توجد سلف' : 'No loans recorded'"
          :empty-description="locale === 'ar'
            ? 'السلف والقروض الممنوحة للموظفين والأقساط المتبقية تظهر هنا.'
            : 'Advances and loans granted to employees with remaining balances appear here.'"
        >
          <template #cell-employee_name="{ value }">
            <span class="text-sm font-medium text-neutral-900 dark:text-neutral-0">{{ value }}</span>
          </template>
          <template #cell-principal_amount="{ value }">
            <span class="font-mono text-sm tabular-nums text-neutral-900 dark:text-neutral-0" dir="ltr">{{ formatMoney(value) }}</span>
          </template>
          <template #cell-monthly_installment="{ value }">
            <span class="font-mono text-sm tabular-nums text-neutral-700 dark:text-neutral-200" dir="ltr">{{ formatMoney(value) }}</span>
          </template>
          <template #cell-outstanding="{ row }">
            <span class="font-mono text-sm font-semibold tabular-nums text-warning-700 dark:text-warning-500" dir="ltr">{{ formatMoney(row.outstanding_balance) }}</span>
          </template>
        </UiDataTable>
      </Can>
    </div>
  </FeatureBoundary>
</template>

<script setup lang="ts">
import Can from '~/core/rbac/Can.vue'
import { PERMISSIONS } from '~/core/rbac/permissions'

definePageMeta({ layout: 'dashboard' })
const { locale } = useI18n()
const api = useApi()
const rows = ref<any[]>([])
const loading = ref(true)

const columns = computed(() => [
  { key: 'employee_name', label: locale.value === 'ar' ? 'الموظف' : 'Employee' },
  { key: 'principal_amount', label: locale.value === 'ar' ? 'أصل المبلغ' : 'Principal', class: 'text-end' },
  { key: 'monthly_installment', label: locale.value === 'ar' ? 'القسط الشهري' : 'Installment', class: 'text-end' },
  { key: 'outstanding', label: locale.value === 'ar' ? 'المتبقي' : 'Outstanding', class: 'text-end' },
])

function formatMoney(n: number | string | null | undefined) {
  return Number(n ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
}

async function load() {
  loading.value = true
  try {
    const r: any = await api.get('/payroll/loans').catch(() => ({ data: [] }))
    rows.value = (Array.isArray(r) ? r : (r.data ?? [])).map((l: any) => ({ ...l, employee_name: l.employee?.name ?? '—' }))
  } catch { rows.value = [] } finally { loading.value = false }
}
onMounted(load)
</script>
